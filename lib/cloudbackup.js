...
window.CloudBackup = (() => {
  ...
  const EXT_BY_KIND = { imagen: 'img', pdf: 'pdf', musicxml: 'musicxml', midi: 'mid', melodia: 'txt', archivo: 'bin' };
  function extForScore(item) {
    if (item.kind === 'imagen') {
      const t = String(item.type || '').split('/')[1];
      return t ? t.replace('jpeg', 'jpg') : 'img';
    }
    return EXT_BY_KIND[item.kind] || 'bin';
  }
  ...
  async function backupNow(withGesture) {
    ...
    try {
      ...
      if (window.ScoreNotes && ScoreNotes.supported && ScoreNotes.supported()) {
        const items = await ScoreNotes.listAll();
        if (items.length) {
          const scoresDir = await handle.getDirectoryHandle('scores', { create: true });
          const manifest = [];
          for (const it of items) {
            const ext = extForScore(it);
            await writeFile(scoresDir, it.itemId + '.' + ext, it.blob);
            manifest.push({ itemId: it.itemId, pieceId: it.pieceId, label: it.label, kind: it.kind, type: it.type, filename: it.filename, ts: it.ts, ext });
          }
          await writeFile(scoresDir, 'manifest.json', JSON.stringify(manifest, null, 2));
        }
      }
  ...
  async function restoreFromFolder(withGesture) {
    ...
    try {
      ...
      if (window.ScoreNotes && ScoreNotes.supported && ScoreNotes.supported()) {
        try {
          const scoresDir = await handle.getDirectoryHandle('scores', { create: false });
          const manifestFile = await readFileIfExists(scoresDir, 'manifest.json');
          const manifest = manifestFile ? JSON.parse(await manifestFile.text()) : [];
          for (const entry of manifest) {
            const fh = await scoresDir.getFileHandle(entry.itemId + '.' + entry.ext, { create: false }).catch(() => null);
            if (!fh) continue;
            const blob = await fh.getFile();
            const added = await ScoreNotes.putIfMissing({
              itemId: entry.itemId, pieceId: entry.pieceId, label: entry.label,
              kind: entry.kind, type: entry.type, filename: entry.filename, ts: entry.ts, blob
            });
            if (added) scores++;
          }
        } catch (e) { /* sin carpeta de partituras todavía: nada que restaurar */ }
      }

      return { ok: true, nuevos, audios, scores };
    } catch (e) {
